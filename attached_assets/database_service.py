...
class DatabaseService:
    ...
    def export_company_data_csv(self, company_name: str) -> Optional[str]:
        ...
        try:
            ...
            for supplier in suppliers:
                tech_list = supplier.technological_differentiators if supplier.technological_differentiators else []
                technologies = ', '.join(tech_list)
                writer.writerow([
                    supplier.segment_name,
                    supplier.supplier_name,
                    supplier.relevance_score if supplier.relevance_score else 0,
                    supplier.innovation_index if supplier.innovation_index else 0,
                    supplier.esg_rating if supplier.esg_rating else 0,
                    supplier.domain if supplier.domain else '',
                    technologies
                ])
    ...
    def cleanup_old_analyses(self, days_old: int = 30) -> int:
        ...
        try:
            ...
            # Get old analyses
            old_analyses = session.query(CompanyAnalysis).filter(
                CompanyAnalysis.created_at < cutoff_date
            ).all()
